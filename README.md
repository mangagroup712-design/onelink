...
Cloudflare Worker で動的に URL を短縮できるようにした URL 短縮サービスです。アクセスしているドメインに合わせて短縮 URL を生成し、`1.max7.work` と `1lk.f5.si` の両方で利用できます。

- 動的短縮（Worker）: `https://1.max7.work/{code}` または `https://1lk.f5.si/{code}`（1〜6文字の英数字）
- 即時短縮（静的）: アクセス中のドメインの `/{payload}`
...
```
https://1.max7.work/{payload}
```

アクセス中のドメインにペイロードを埋め込みます。サーバーや DB を使わず、作成直後からどのデバイスでも利用できます。
...
4. `1.max7.work` と `1lk.f5.si` を Worker の Custom Domain / Route に割り当てる
